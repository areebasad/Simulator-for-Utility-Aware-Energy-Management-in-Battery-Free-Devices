...
from ..predictors import Predictor, EWMA
...
class DepletionSafePlus(EnergyManager):    
    ...
    def __init__(self, config: Dict[str, Any], buffer, scheduler):
        ...
        predictors = {
           "ideal" : Predictor, # base
           "ewma" : EWMA, # EWMA
        }
        self.predictor = predictors[config["predictor"]](config)
        self.buffer = buffer
    ...
    def findBestIn(self, startSlot, numSlots, startVc):
        ...
        if startVc < self.cap_vc_min: #or startVc <  self.min_goal:
            return minIn_amp, In_amp, voltage_forecast
            ...
            for wi in weight_indices:
                ...
                while abs(maxIn_amp - minIn_amp) > self.simulation_ibal_tolerance: #If search window gets so small then we break
                    ...
                    for ds in range(numSlots):
                        ...
                        if highest_weightage_slot == weight_slot:
                            # Non-linear Function
        ...
        return rec_In_amp, In_amp, voltage_forecast # Return in Amperes
        ...
        if isDeviveOn == True:
            ...
            if df.at[state, "is_planning_slot"] == True or self.planning_step_counter >= self.next_planning_after_x_steps:
                ...
                if state < 288: # For first day, do minimum
                    ...
                    budget_current_amp, In_forecast, voltage_forecast = self.findBestIn(self.planning_slot_counter, self.planning_number_of_slots_per_day , start_buffer_voltage)
                    budget_current_mA = 1000 * budget_current_amp
                    In_forecast_mA = [x*1000 for x in  In_forecast]
                    run_planning = True
                    df.at[state, "dsp_consp_forecast"] = str(In_forecast_mA)
                    df.at[state, "dsp_voltage_forecast"] = str(voltage_forecast)
