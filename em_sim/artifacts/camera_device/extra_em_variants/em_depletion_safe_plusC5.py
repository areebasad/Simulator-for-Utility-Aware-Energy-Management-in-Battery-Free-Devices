...
class DepletionSafePlus(EnergyManager):    
    ...
    def __init__(self, config: Dict[str, Any], buffer, scheduler):
        ...
        super().__init__(config, "Energy Manager: UtiliGEM")
    ...
    # Multi Raise and then Single Raise Binary Search
    def multiSingleRaiseBS(self, startSlot, numSlots, startVc, weights, weight_indices, required_max_budget_amp, list_In_amp, multiRaise=True ):

        indices_to_skip = []
        max_weight = max(weights)
        #if (max(weights) - min(weights)) != 0:

        In_amp = list_In_amp.copy()  # Current in each slot

        # loop to iterate over slots in priority order
        for weight, w_index in weight_indices:
            ...
            assesing_slots = self.get_indices_by_weight(weight_indices, weight, max_weight)
            assesing_slots = self.remove_elements(indices_to_skip, assesing_slots)

            # Combine indices_with_target_weight with skip_indices
            indices_to_skip += assesing_slots
           # else: # SingleRaise
           #     assesing_slots = []
           #     assesing_slots.append(w_index)            

            if len(assesing_slots) > 1 and multiRaise == True:
                # Binary Search: MultiRaise
                # Set binary search boundries
                maxIn_amp = required_max_budget_amp # upper bound on application consumption
                minIn_amp =  In_amp[w_index] # lower bound in amperes

                # Binary Search Multi Raise
                while abs(maxIn_amp - minIn_amp) > self.simulation_ibal_tolerance: #If search window gets so small then we break
                    ...
                    for ds in range(numSlots):
                        ...
                        if current_slot in assesing_slots:
                            # Non-linear Function
                            ...
                            Vc = self.simulate_newton(
                                ...
                                Vc,
                                self.predictor.predict(predictor_slot)/ 1000, 
                                In_amp[current_slot] * self.dc_dc_voltage, self.cap_eta)
                ...
                # Update BS result in In_amp for all assesing slots 
                for index in assesing_slots:
                    In_amp[index] = minIn_amp
        
            # Binary Search Single Raise
            for assesing_slot in assesing_slots:

                # Set binary search boundries
                maxIn_amp = required_max_budget_amp # upper bound on application consumption
                minIn_amp =  In_amp[assesing_slot] # lower bound in amperes
                ...
                # Update BS result in In_amp for assesing slot 
                In_amp[assesing_slot] = minIn_amp
    ...
    def findBestIn(self, startSlot, numSlots, startVc):
        ...
        # no operation, if node is off or currently below drop level, extra comment: use mingoal to avoid failures. 
        ...
        if (vam_convg == True) and (requiredIn_amp < maxIn_amp):
            ...
            pass1_In_amp =  self.multiSingleRaiseBS(startSlot, numSlots, startVc, weights, weight_indices, required_max_budget_amp, In_amp, multiRaise=True )

            # Then we again do multi raise bs with  maxIn and In that we found from pass 1
            required_max_budget_amp = maxIn_amp
            pass2_In_amp =  self.multiSingleRaiseBS(startSlot, numSlots, startVc, weights, weight_indices, required_max_budget_amp, pass1_In_amp, multiRaise=True)
            ...
            # Send result
            rec_In_amp = pass1_In_amp[startSlot]
