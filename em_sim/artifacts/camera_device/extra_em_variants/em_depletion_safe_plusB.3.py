...
# Version B.3
# Areebs way -> do binary search with weightages but check B_end state as well
# then do progressive filling check B_end state
...
class DepletionSafePlus(EnergyManager):    
    ...
    def all_slots_at_max(self, weightages, MaxIn, startSlot, saturation_slot, numSlots):
        ...
        slots_to_check = [(startSlot + ds) % numSlots for ds in range((saturation_slot - startSlot))]
        
        return all([MaxIn * weightages[slot] >= MaxIn for slot in slots_to_check])

    def progressive_filling(self, binary_In, startVc, startSlot, saturation_slot, numSlots, reference_voltage ):
    ...
    def findBestIn(self, startSlot, numSlots, startVc):
        ...
        # Check if capacitor is greater than 60F (i.e. more than consumption storage)
        ...
        # Store startVc0 
        if startSlot == 0:
            self.startVc0 = 1.8 #startVc and buffer end Vc 
        ...
        while abs(maxIn_amp - minIn_amp) > self.simulation_ibal_tolerance:
            ...
            for ds in range(numSlots):
                slot = startSlot + ds
                current_for_slot_amp = In * current_weightages[slot % numSlots]
                ...
                Vc[(slot+1) % numSlots] = self.simulate_newton(
                    ...
                    current_for_slot_amp * self.dc_dc_voltage, self.cap_eta)
                ...
                tolerance = 0.02  # for example
                # Ensuring buffer end state is greater than or approximately equal to buffer start state
                if slot == (numSlots-1) and (Vc[(numSlots-1)] < self.startVc0 and (abs(Vc[(numSlots-1)] - self.startVc0) > tolerance)):
                    no_violation = False
                    break
        ...
        if minIn_amp >= (self.util_max_current_mA * 1e-3):
            ...
            '''
            #if startSlot == (numSlots-1):
            #    recommended_budget_current_amp = minIn_amp * current_weightages[startSlot]
            #    minIn_amp = recommended_budget_current_amp
            #else:
            saturation_slot = 24 #numSlots # Saturation slot is the end slot
            reference_voltage = self.startVc0
            new_weightages  = self.progressive_filling(minIn_amp, startVc, startSlot, saturation_slot, numSlots, reference_voltage)
