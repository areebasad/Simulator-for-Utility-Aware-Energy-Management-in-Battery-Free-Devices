...
# B: Equal weights
# 1. Do DS
#Difference from C1 and C2, we have a end state condition
# if initial is >50 then end with >50, if not then end with any state
        ...
        if (max(weights) - min(weights)) != 0:
            ...
            for wi in weight_indices:
                ...
                while abs(maxIn_amp - minIn_amp) > self.simulation_ibal_tolerance: #If search window gets so small then we break
                    ...
                    # End of slot simulations
                    if startVc > 1.985:
                        if ok and Vc > 1.985:
                            minIn_amp = In  # Increase min boundary if conditions are met
                        else:
                            maxIn_amp = In  # Decrease max boundary
                    else:
                        if ok:
                            minIn_amp = In  # Increase min boundary based only on ok
                        else:
                            maxIn_amp = In  # Decrease max boundary
