...
# Set self.planning_slot_counter-1  for budget DS and free region in emrep_step
...
# Simulate excess case for camera artifact
# A) Camera settings for learned utility profile, with comment dev_min 1.5 in findMaxInTillFull
# B) For Maximum Utility, use following
# Change planning_horizon_ms to 1 hour/slot in iniate func 
# Change self.Vc = Vc in findbestIn func
# Set self.planning_slot_counter-1  for budget DS and free region in emrep_step
# Set self.planning_slot_counter  for recalc regioun in getLimited func
# Set  self.checkUpTimeFac = 1e4 or 1e8 in initiate func
# comment dev_min 1.5 in findMaxInTillFull
...
class EmRep(EnergyManager):    
    ...
    def __init__(self, config: Dict[str, Any], buffer, scheduler):
        ...
        self.cap_eta = 0.8                                  # This is different to what we assume in simulation
        ...
        self.decouplingSoc = 0.05   # 5% as suggested by authors, 0.05
        self.checkUpTimeFac = 1e4   # Lars value: 1e6, Camera case value, 1e4, In case of U-Max Profile values lower than 1e6 dont make any difference, 1e8 keeps stable. 1e4 = 10000
        self.recalcDur = 24         # 24 Slots
        self.Ilim = 100             # Lars Value: 100 
        ...
        if self.current_time_ms >= self.timeLastSchedUpdate + self.schedValidity:
            ...
            if Vc >= self.min_goal:
                if self.budgetRegion == BUDGET_REGION.REGION_DS:
                    ...
                    self.budget = self.getLimitedBudget(self.budgetFactor, self.planning_slot_counter-1) # Current slot for Lars case, prev slot for Camera case
    ...
    def findBestIn(self, startSlot, numSlots, startVc):
        ...
        #self.Vc = Vc # Is this intented? Its in the lars code and unintentionaly getting changed. 
